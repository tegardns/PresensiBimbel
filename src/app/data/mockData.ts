...
export const levels: Level[] = [
  { id: 'LVL-001', nama: 'Calistung', harga: 40000, durasi: [60, 90, 120], status: 'aktif' },
  { id: 'LVL-002', nama: 'SD', harga: 50000, durasi: [60, 90, 120], status: 'aktif' },
  { id: 'LVL-003', nama: 'SMP', harga: 60000, durasi: [60, 90, 120, 150, 180], status: 'aktif' },
  { id: 'LVL-004', nama: 'SMA', harga: 70000, durasi: [60, 90, 120, 150, 180], status: 'aktif' },
];
...
export const mapels: Mapel[] = [
  ...
  { id: 'MAP-010', nama: 'Bahasa Inggris', levelId: 'LVL-002', status: 'aktif' },
  { id: 'MAP-011', nama: 'Bahasa Inggris', levelId: 'LVL-003', status: 'aktif' },
  { id: 'MAP-012', nama: 'Bahasa Inggris', levelId: 'LVL-004', status: 'aktif' },
];
...
export const getLevelByName = (name: string) => levels.find(l => l.nama === name);
...
export const getPresensiById = (id: string) => presensis.find(p => p.id === id);
...
export const calculateAdminCommission = (levelId: string, durasi: number): number => {
  const level = getLevelById(levelId);
  if (!level) return 0;
  const hargaPerMenit = level.harga / 60;
  const total = hargaPerMenit * durasi;
...
export const getPresensiByTutor = (tutorId: string) => presensis.filter(p => p.tutorId === tutorId);
export const getPresensiByStatus = (status: Presensi['status']) => presensis.filter(p => p.status === status);
export const getPayoutByTutor = (tutorId: string) => payouts.filter(p => p.tutorId === tutorId);
...
export const getStatistics = () => {
  ...
  const monthPresensis = presensis.filter(p => p.tanggal.startsWith(currentMonth) && p.status === 'selesai');
